...
public class ChatMsgDao {
	...
	public int insert(Msg msg) {
		...
		values.put(DBcolumns.MSG_FROM_USER,msg.getFromUserName());
	...
	public ArrayList<Msg> queryMsg(String from,String to,int offset) {
		...
		String sql = "select * from " + DBcolumns.TABLE_MSG +
				" where ("+DBcolumns.MSG_FROM+"=? and "+DBcolumns.MSG_TO+"=?) or("+DBcolumns.MSG_FROM+"=? and "+DBcolumns.MSG_TO+"=?)"
				+ "order by " + DBcolumns.MSG_ID + " desc limit ?,?";
		String[] args = new String[] {from,to,to,from,String.valueOf(offset),"15" };
	...
	public long updateAllMsgToRead(String from,String to) {
		...
		String whereCluase = "(" + DBcolumns.MSG_FROM+ " =? and "+DBcolumns.MSG_TO+ " =? and "+DBcolumns.MSG_ISREADED+"=0) "
				+ "OR (" + DBcolumns.MSG_FROM+ " =? and "+DBcolumns.MSG_TO+ " =? and "+DBcolumns.MSG_ISREADED+"=0) ";
		long row = db.update(DBcolumns.TABLE_MSG, values, whereCluase, new String[] { from,to, to, from });
		db.close();
		return row;
	}
	 ...
	 * @param msgId
	 */
	public long updateOneMsgToRead(int msgId){
		SQLiteDatabase db = helper.getWritableDatabase();
		ContentValues values = new ContentValues();
		values.put(DBcolumns.MSG_ISREADED, "1");
        long row = db.update(DBcolumns.TABLE_MSG,values,DBcolumns.MSG_ID + " =?",new String[]{String.valueOf(msgId)});
        db.close();
		return row;
	}
